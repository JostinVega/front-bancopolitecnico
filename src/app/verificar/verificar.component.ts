...
export class VerificarComponent {
  ...
  onSubmit(): void {
    ...
    const verificationCode = this.digits.join('');
    this.http.post('https://back-enviar-codigos-validacion.vercel.app/verify-code', { email, code: verificationCode }).subscribe(() => {
      this.router.navigateByUrl('/crear-usuario');
    }, () => {
      this.attempts++;
      if (this.attempts >= 3) {
  ...
  resendCode(): void {
    const email = this.registroService.getRegistrationData('step1').correo_electronico;
    this.http.post('https://back-enviar-codigos-validacion.vercel.app/send-code', { email }).subscribe(() => {
